...
function App() {
  ...
  if (started) {
    return <div className="app"><header><div className="logo">R</div><div><h1>Ram Chat</h1><p>Simple • Fast • Private</p></div></header><main><section className="card">
      ...
      <button type="button" onClick={() => alert('Authentication will be connected next.')}>{mode === 'login' ? 'Login' : 'Sign Up'}</button>
      <button className="secondary" type="button" onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}>{mode === 'login' ? 'Create new account' : 'Already have an account? Login'}</button>
    </section></main></div>;
  }
